...
def visualize_model_architecture(model_name):
    model_options = {
        "3D U-Net": build_3d_unet,
        "3D V-Net": build_3d_vnet,
        "SA-Net": build_sa_net,
        "E1D3 U-Net": build_e1d3_unet,
        "HDC Net": build_3d_hdc_net
    }

    model = model_options[model_name](input_shape=(64, 64, 64, 1))
    ...
    plot_path = f"{model_name.replace(' ', '_')}_plot.png"
    plot_model(model, to_file=plot_path, show_shapes=True, dpi=200)

    with st.expander(f"🔍 View {model_name} Details"):
        st.markdown("### 📄 Model Summary")
        st.code(summary_str, language="bash")
